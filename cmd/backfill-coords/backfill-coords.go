...
func writeSQL(w io.Writer, decisions []decision) {
...
func writeReport(w io.Writer, decisions []decision) {
...
// dashes builds the report's header rule: one run of dashes per column,
// separated by the same two spaces the column format string uses.
