...
def test_embed_then_find(db_conn, embedder, video_id):
    ...
    # Disable dedup + black-skip so the counts are exact.
    opts = {"apply": True, "dedup_threshold": 2.0, "black_threshold": 0.0}
    result = embed_video(db_conn, embedder, ref, interval_sec=10.0, **opts)
    ...
    again = embed_video(db_conn, embedder, ref, interval_sec=10.0, **opts)
