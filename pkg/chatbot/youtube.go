...
// gatewayYouTubeChat is the outbound chat client for a gateway-wired youtube
// instance: it posts through gateway-youtube's SendChat, which resolves the
// channel's active live chat itself. Unlike youtubeChat it holds no live-chat
// binding (and no YouTube token) — the gateway owns the send and the chat-ID
// resolution. Implements the provider-neutral ChatClient seam.
type gatewayYouTubeChat struct {
	client *gateway.Client
}

func (g gatewayYouTubeChat) Say(msg string) {
	// Twitch-only IRC emote prefix; literal text on YouTube (see youtubeChat.Say).
	msg = strings.TrimPrefix(msg, "/me ")
