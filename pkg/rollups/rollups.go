...
// subscriber bonus and historical manual corrections, so it reads lower than
// users.miles (that delta is a drift alarm, not a bug). user_rollups.extra_miles
// captures SUM(events.extra_miles_earned): the sub-grant + 5%-bonus portion the
// pairing can't see, so reconstructed display miles ≈ events_miles + extra_miles.
